...
class JoinerSpec extends FlatSpec {
  ...
  "With no input on P1, a gated joiner " should " return nothing" in {
    val graph = Jafpl.newInstance().newGraph()
    val bc = new BufferSink()
    ...
    graph.addGatedEdge(p1, "result", pipeline, "result")
    graph.addEdge(p2, "result", pipeline, "result")
    graph.addEdge(p3, "result", pipeline, "result")
    graph.addEdge(pipeline, "result", consumer, "source")
  ...
  "With regular input on P1, a gated joiner " should " return input from P1" in {
    val graph = Jafpl.newInstance().newGraph()
    val bc = new BufferSink()
  ...
  "With gating input on P1, a gated joiner " should " return B* and C*" in {
    val graph = Jafpl.newInstance().newGraph()
    val bc = new BufferSink()

    val pipeline = graph.addPipeline(Manifold.ALLOW_ANY)
    val p1 = pipeline.addAtomic(new Producer(List("GATE_MESSAGE")), "Adocs")
    val p2 = pipeline.addAtomic(new Producer(List("B1","B2")), "Bdocs")
    val p3 = pipeline.addAtomic(new Producer(List("C1","C2")), "Cdocs")
    val consumer = pipeline.addAtomic(bc, "consumer")
